...
\begin{equation}
  c = (\text{tree\_hash},\; \text{pipeline\_id},\; T,\; v,\; \text{parents},\;
  \text{env\_constraints},\; \text{evidence\_refs}),
\end{equation}
where $\text{tree\_hash}$ is the root hash of the working directory tree (same
role as Git's tree object), $\text{pipeline\_id}$ is the content hash of the
...
Concretely, \texttt{morph merge --retire $m_1,\dots,m_k$} enforces this
requirement: if the user-supplied pipeline already carries a \texttt{review}
node, it is used unchanged; otherwise the merger synthesizes one whose
\texttt{params} list the retired metrics and the reason supplied via
\texttt{--retire-reason} (or a generic placeholder when omitted), and whose
attribution points at the merge author. The synthesized node's identifier is
derived deterministically from $(D, \text{reason}, \text{author})$, so an
identical retirement collapses to the same node across reruns while distinct
retirements get distinct ids. The same enforcement applies to long-running
merges that pause for conflict resolution: \texttt{morph merge feature
--retire $m_1,\dots,m_k$ --retire-reason \dots} persists $D$ and the reason
as a \texttt{MERGE\_RETIRED.json} breadcrumb under \texttt{.morph/}, and a
later \texttt{morph merge --continue} reads the breadcrumb to inject the
same attributed \texttt{review} node before commit construction.
\texttt{morph merge --abort} clears the breadcrumb alongside the rest of
the merge state.
...
happened at every merge. Commands like \texttt{morph session import} and
...
\texttt{morph clone <url>} & Initialize from a remote \\
...
\texttt{morph session import <file>} & Ingest execution receipt \\
\texttt{morph eval run -- <cmd>} & Run tests, parse, and record evidence \\
\texttt{morph eval record <file>} & Ingest evaluation results \\
\texttt{morph eval add <path>} & Register acceptance case in default suite \\
\texttt{morph annotate <hash>} & Attach metadata \\
\texttt{morph log} / \texttt{status} & Inspect history and state \\
\texttt{morph push <remote> <branch>} & Send commits + evidence to a remote \\
\texttt{morph fetch <remote>} & Pull remote refs into tracking branches \\
\texttt{morph pull <remote> <branch>} & Fetch + fast-forward (or merge) \\
\texttt{morph sync [branch]} & Fetch + pull against the configured upstream \\
\end{tabular}
\end{center}

\paragraph{Auto-attached evidence.}
\textsc{Morph} optimizes for the common eval-driven workflow---run tests, then
commit---by leaving a small \texttt{LAST\_RUN.json} breadcrumb under
\texttt{.morph/} whenever \texttt{morph eval run} or
\texttt{morph eval from-output --record} persists a metric-bearing run. The
breadcrumb stores the run hash, HEAD at record time, and a fingerprint of the
staging index. A subsequent \texttt{morph commit -m "msg"} (with neither
\texttt{--from-run} nor \texttt{--metrics} supplied) reads the breadcrumb,
verifies that HEAD and the index still match, and---if so---auto-attaches the
run's metrics to the commit's eval contract and the run hash to its
\texttt{evidence\_refs}. A stale breadcrumb (HEAD or index changed since
recording) is ignored with a stderr nudge so the user can see why their
evidence was not attached. The breadcrumb is cleared after a successful commit
so the same run is never silently re-attached, and \texttt{--no-auto-run}
disables the pickup entirely for audited metrics-less commits. The merge gate
is unaffected: it only sees the resolved \texttt{Run} the breadcrumb pointed
at, identical to what \texttt{--from-run <hash>} would have produced.

\paragraph{Auto-detected acceptance cases.}
\textsc{Morph} also auto-fills \texttt{introduces\_cases} annotations. When
\texttt{morph commit} is run without an explicit \texttt{--new-cases} flag, it
diffs the about-to-commit eval suite against HEAD's suite and records any
case ids present in the new suite but absent in the parent's---typically the
acceptance cases the user just registered with \texttt{morph eval add}.
Pass \texttt{--new-cases ""} to suppress the diff, or
\texttt{--new-cases "id1,id2"} to override it with a manual list. The
annotation feeds merge planning's case provenance summary unchanged.
...
implementation already exposes that model: \texttt{morph clone},
\texttt{morph push}, \texttt{morph fetch}, \texttt{morph pull}, and
\texttt{morph sync} move the full object graph---commits, pipelines,
evaluation suites, runs, traces, and annotations---over an SSH-based
transport, with optional server-side push gates that refuse a ref update
when the candidate commit fails the receiving repo's behavioral policy.
What remains future work is federated verification across mutually
distrustful hosts: signed commits, third-party run attestations, and
cross-repo replay of evaluation suites without re-trusting the producer.
...
Second, structural merge of pipeline DAGs (step 1 of the merge procedure)
has a working implementation but no formal correctness theory.
\textsc{Morph} v0 reconciles pipelines per-node by stable id (not by
hash): nodes present on both sides with identical bodies merge cleanly,
side-only nodes are unioned, and conflicting bodies are surfaced as
\textit{add/add}, \textit{modify/modify}, or \textit{modify/delete}
conflicts that drop into a \texttt{morph merge} state machine for
explicit resolution. Trees merge per-path with a text leaf and the same
conflict shape; eval suites auto-union when metric definitions agree and
surface a structural conflict otherwise. That covers the practical cases.
What is still open is a correctness statement for graph-level merge:
when two branches modify the same prompt node in different directions,
\textsc{Morph} can record a deterministic resolution but cannot yet
\emph{prove} the merged pipeline is the right one. Three-way text merge
has decades of theory behind it; pipeline-graph merge does not.
